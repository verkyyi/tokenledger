...
export const en = {
  ...
  'kpis.perM': '$ per 1M output',
  'kpis.perM.one': '{source}: metered cost per million output tokens.',
  'kpis.perM.subscription': "{source} is subscription work: it is billed monthly, not per token, so there is no per-million rate to report. The plan's cost is in real spend.",
  'kpis.perM.many': 'This scope spans more than one source. A cost-per-token rate is only meaningful within one — filter by source to see it.',
  'kpis.realSpend': 'real spend',
  'kpis.realSpendTip': '{subscription} subscription + {gateway} gateway = {total}.',
  ...
  'efficiency.subagent': 'subagent',
  'efficiency.tokensTurns': '{tokens} · {events} turns',
  'efficiency.turnsOnly': '{events} turns',
  ...
  /* ----------------------------------------------------------- model mix */
  'modelMix.title': 'Model mix over time',
  'modelMix.hint': 'Tokens per bucket by model, over the selected window (same buckets as the timeline).',
  'modelMix.model': 'Model',
